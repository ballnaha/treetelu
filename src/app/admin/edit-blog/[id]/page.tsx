...
import { useState, useRef, useEffect, ChangeEvent, FormEvent, ReactNode } from 'react';
import { 
  Box, 
  Container, 
  Typography, 
...
export default function EditBlogPage() {
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'title') {
      // สร้าง slug อัตโนมัติจากชื่อบทความ (เฉพาะเมื่อเป็นบทความใหม่)
      if (!postData.id) {
  ...
  const handleDeleteImage = async () => {
    // เก็บ URL รูปภาพปัจจุบันไว้ตรวจสอบ
    const currentImage = postData.image;
    
    // ตรวจสอบว่าเป็นรูปภาพที่อัปโหลดเอง (ไม่ใช่ placeholder)
    if (currentImage && currentImage !== '/images/blog/placeholder.jpg' && currentImage.startsWith('/images/blog/')) {
      try {
        // ดึงชื่อไฟล์จาก URL
        const fileName = currentImage.split('/').pop();
        
        // ส่งคำขอลบไฟล์ไปที่ API
        const response = await fetch(`/api/upload?filename=${fileName}`, {
          method: 'DELETE',
        });
        
        if (!response.ok) {
          console.error('ไม่สามารถลบไฟล์รูปภาพได้:', await response.text());
        } else {
          console.log('ลบไฟล์รูปภาพเรียบร้อยแล้ว:', fileName);
        }
      } catch (error) {
        console.error('เกิดข้อผิดพลาดในการลบไฟล์รูปภาพ:', error);
      }
    }
    
    // เปลี่ยนรูปภาพเป็น placeholder ไม่ว่าการลบไฟล์จะสำเร็จหรือไม่
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // ส่งข้อมูลไปยัง API เพื่ออัปเดตบทความ
        ...
        <Paper sx={{ p: 4, mb: 4 }}>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            ...
            <TextField
              ...
              margin="normal"
              InputProps={{
                readOnly: !postData.id, // อนุญาตให้แก้ไข slug ได้เมื่อเป็นการแก้ไขบทความ
              }}
              helperText="Slug ใช้สำหรับ URL ของบทความ"
            />
            ...
            <Box sx={{ mt: 3, mb: 4 }}>
              ...
              >
                <Image
                  src={postData.image || '/images/blog/placeholder.jpg'}
                  alt="รูปภาพบทความ"
                  fill
