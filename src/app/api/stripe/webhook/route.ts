...
const sendOrderConfirmationEmail = async (
  ...
  try {
    ...
    if (
      ...
      orderData.stripePaymentMethodType === "promptpay"
    ) {
      paymentMethodText = "พร้อมเพย์ (Stripe)";
    } else if (
      orderData.paymentMethod === "CREDIT_CARD" ||
      orderData.stripePaymentMethodType === "card"
    ) {
  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (order) {
          console.log(
            `Found order with ID: ${order.id}, orderNumber: ${order.orderNumber}, updating payment status`
          );
          ...
          if (metadata.payment_method_types) {
            ...
            const paymentMethodTypes =
              typeof metadata.payment_method_types === "string"
          ...
          try {
            ...
            if (!emailResult.success) {
              console.warn(
                "Payment confirmed but email sending failed:",
                emailResult.message
              );
            }
          } catch (emailError) {
          ...
          if (process.env.DISCORD_WEBHOOK_URL) {
            try {
              ...
              if (paymentInfo) {
                ...
                if (
                  paymentInfo.transactionId === sessionData.id &&
                  paymentInfo.status === "CONFIRMED"
                ) {
                  console.log(
                    `Discord notification already sent for order ${order.id} with session ${sessionData.id}`
                  );
                  discordSent = true;
                }
              }
              ...
              if (!discordSent) {
                console.log(
                  `Sending Discord notification for order ${order.id} with session ${sessionData.id}`
                );
                ...
                const paymentEmbed =
                  await createOrderNotificationEmbed(orderDataForDiscord);

                console.log(
                  "Sending Discord notification for Stripe payment - DIRECT",
                  {
                ...
                if (discordResult) {
                  ...
                  if (paymentInfo) {
                    ...
                    console.log(
                      `Updated payment info ${paymentInfo.id} with discord notification status for order ${order.id}`
                    );
                  }
                }
              }
              ...
              console.error(
                "Error in Discord notification process:",
                discordError
              );
            }
          }
          ...
          console.log(
            `No order found for session ID: ${sessionData.id} or in metadata`
          );
          ...
          try {
            ...
            await prisma.pendingPayment.create({
              data: {
                ...
                payment_method: sessionData.payment_method_types?.includes(
                  "promptpay"
                )
            ...
            console.log(
              `Created pending_payment for session ID: ${sessionData.id}`
            );
          } catch (pendingPaymentError) {
            console.error(
              "Error creating pending_payment:",
              pendingPaymentError
            );
          }
          ...
          return NextResponse.json(
            {
              message:
                "Payment received but no order found. Created pending_payment record.",
            },
            { status: 200 }
          );
        }
      ...
      case "payment_intent.succeeded": {
        ...
        console.log(
          `Webhook: Payment intent succeeded: ${paymentIntent.id}, method: ${paymentMethod}`
        );
        ...
        if (order) {
          console.log(
            `Found order: ${order.orderNumber} for payment intent: ${paymentIntent.id}, updating payment status`
          );
          ...
          try {
            ...
            console.log(
              "Sending email notification for payment_intent.succeeded"
            );
            const emailResult = await sendOrderConfirmationEmail(orderData);
            if (!emailResult.success) {
              console.warn(
            ...
            try {
              ...
              if (paymentInfo) {
                ...
                if (
                  ...
                  console.log(
                    `Discord notification already sent for order ${order.id} with payment intent ${paymentIntent.id}`
                  );
                  discordSent = true;
                }
              }
              ...
              if (!discordSent) {
                console.log(
                  `Sending Discord notification for order ${order.id} with payment intent ${paymentIntent.id}`
                );
                // ตรวจสอบว่าเป็น promptpay หรือ card จริงๆ
                const isPromptpay =
                  paymentMethodType === "promptpay" ||
                ...
                console.log(
                  "Sending Discord notification for payment_intent.succeeded - DIRECT",
                  {
                    webhookUrl: !!process.env.DISCORD_WEBHOOK_URL,
                    orderNumber: order.orderNumber,
          ...
          console.log(
            `No order found for payment intent: ${paymentIntent.id}, checking pending_payments...`
          );
          ...
          if (!pendingPayment) {
            console.log(
              `Creating new pending payment for payment intent: ${paymentIntent.id}`
            );
            ...
            console.log(
              `Updating existing pending payment: ${pendingPayment.id}`
            );
