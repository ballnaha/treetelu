...
/**
 * POST - ตรวจสอบความถูกต้องของโทเค็นรีเซ็ตรหัสผ่าน
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error in verify reset token endpoint:', error);
    
    return NextResponse.json(
      { success: false, message: 'เกิดข้อผิดพลาดในการตรวจสอบโทเค็น' },
      { status: 500 }
    );
  }
}